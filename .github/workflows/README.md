...
Only the parts that changed are rebuilt — backend pushes don't rebuild the SPA
and vice versa.
...
| `GCP_DEPLOYER_SERVICE_ACCOUNT` | `ci-deployer@steel-shine-492401-u6.iam.gserviceaccount.com` — must have `roles/run.admin`, `roles/artifactregistry.writer`, `roles/iam.serviceAccountUser` |
| `FIREBASE_SERVICE_ACCOUNT` | JSON key for a service account in the frontend project with `roles/firebasehosting.admin` |
| `VITE_API_BASE` | (optional) Override the frontend's API URL. Default is `https://placeup-api-rui2a74muq-ue.a.run.app`. |

### Why Workload Identity Federation instead of long-lived keys?

GCP service-account JSON keys are bearer tokens that don't expire. Leaking one
in a GitHub Actions log is a project-wide compromise. WIF lets the workflow
mint a short-lived token from its OIDC identity — no secret to rotate, no key
to exfiltrate.
...
re-deploy without pushing a commit. You can also skip the backend or frontend
half from the input form.
