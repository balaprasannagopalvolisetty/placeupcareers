/**
 * Country reference data for the signup flow.
 *
 * `COUNTRIES` powers the international phone-code picker (flag + dial code +
 * name) and the Country dropdown. The most common visa-sponsorship
 * destinations are pinned to the top, then every other country alphabetically.
 *
 * `visaOptionsForCountry(code)` returns the work-authorization / visa status
 * options to show for a given country. Every list ends with "Other", which the
 * signup form turns into a free-text box.
 *
 * Generated from pycountry + libphonenumber; safe to edit by hand.
 */

export interface Country {
  code: string;   // ISO 3166-1 alpha-2
  name: string;
  dial: string;   // E.164 dial code, e.g. "+1"
...
// Default phone country for the picker.
export const DEFAULT_PHONE_COUNTRY = "US";

// ── Work-authorization / visa status options, per country ──────────────
// Keyed by ISO2. Countries without a specific list fall back to GENERIC.
const GENERIC_VISA: string[] = [
  "Citizen",
  "Permanent Resident",
...
const VISA_OPTIONS_BY_COUNTRY: Record<string, string[]> = {
  US: [
    "F1",
    "F1-OPT",
    "F1-STEM OPT",
