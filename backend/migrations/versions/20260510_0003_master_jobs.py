...
def upgrade() -> None:
    ...
    bind = op.get_bind()
    if not sa.inspect(bind).has_table("master_jobs"):
        op.create_table(
            "master_jobs",
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_master_jobs_status_seen ON master_jobs (status, last_seen_at)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_master_jobs_company ON master_jobs (company)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_master_jobs_source ON master_jobs (source_name)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_master_jobs_visa ON master_jobs (visa_score)")
