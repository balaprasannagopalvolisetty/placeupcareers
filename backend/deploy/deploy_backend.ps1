...
$ApiEnv = "APP_ENV=production,DATABASE_BACKEND=postgres,USER_DATABASE_BACKEND=$UserDatabaseBackend,USER_FIRESTORE_PROJECT_ID=$UserFirestoreProjectId,USER_FIRESTORE_DATABASE=$UserFirestoreDatabase,SCRAPE_INTERVAL_HOURS=8,SCRAPEGRAPH_ENABLED=false,ADMIN_EMAILS=jobs@placeupcareer.com"
if ($FrontendUrl) {
  $ApiEnv = "$ApiEnv,FRONTEND_URL=$FrontendUrl"
}
$ScraperEnv = "APP_ENV=production,DATABASE_BACKEND=postgres,SCRAPE_INTERVAL_HOURS=8,SCRAPEGRAPH_ENABLED=false,SCRAPEGRAPH_DISCOVERY_ENABLED=false,SCRAPEGRAPH_DISCOVERY_MAX_URLS=220,SCRAPEGRAPH_DISCOVERY_CONCURRENCY=3,SCRAPE_MAX_CONCURRENCY=10,SCRAPER_PUBLIC_SOURCES=linkedin~indeed~glassdoor~ziprecruiter~google~rapidapi~usajobs~dice,SCRAPER_ROLE_BATCH_SIZE=8,SCRAPER_PUBLIC_BATCH_CONCURRENCY=2,API_CONNECTOR_SOURCES=adzuna~greenhouse~remoteok~remotive~jobicy,SCRAPE_GLASSDOOR_JOBSPY_ENABLED=true,SCRAPE_ZIPRECRUITER_JOBSPY_ENABLED=true,RAPIDAPI_REQUEST_DELAY_SECONDS=3,RAPIDAPI_RATE_LIMIT_COOLDOWN_SECONDS=900,LINKEDIN_REQUESTS_PER_MINUTE=4,LINKEDIN_THIN_DESCRIPTION_CHARS=1200,LINKEDIN_ENRICH_MAX_JOBS_PER_RUN=500,LINKEDIN_ENRICH_CONCURRENCY=1"
