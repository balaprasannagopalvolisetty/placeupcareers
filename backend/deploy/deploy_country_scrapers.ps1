param(
  ...
  [int]$RolePipelineCount = 117,
  [int]$RoleParallelism = 6
)
...
$baseEnv = "APP_ENV=production,DATABASE_BACKEND=postgres,DB_POOL_SIZE=2,DB_MAX_OVERFLOW=2,SCRAPE_INTERVAL_HOURS=6,SCRAPEGRAPH_ENABLED=false,SCRAPEGRAPH_DISCOVERY_ENABLED=false,SCRAPLING_DISCOVERY_MAX_TARGETS=120,SCRAPLING_H1B_EXCEL_COMPANY_LIMIT=200,SCRAPLING_DISCOVERY_CONCURRENCY=2,SCRAPE_MAX_CONCURRENCY=3,SCRAPER_PUBLIC_SOURCES=linkedin~indeed~glassdoor~ziprecruiter~google~usajobs~dice,SCRAPER_ROLE_BATCH_SIZE=4,SCRAPER_PUBLIC_BATCH_CONCURRENCY=1,SCRAPER_PUBLIC_MAX_BATCHES_PER_RUN=0,SCRAPER_RUN_BUDGET_SECONDS=12600,SCRAPER_RECENCY_HOURS=24,SCRAPER_ROLE_MATRIX_ENABLED=true,SCRAPER_ROLE_PIPELINE_COUNT=$RolePipelineCount,SCRAPER_MATRIX_DIRECT_ATS_ENABLED=true,SCRAPER_PROVIDER_BLOCK_COOLDOWN_SECONDS=1800,SCRAPER_PROVIDER_EMPTY_CIRCUIT_THRESHOLD=4,SCRAPER_PURGE_EXCEPT_TODAY=false,SCRAPER_COVERAGE_FLOOR_ENABLED=false,SCRAPER_BOARD_PASS_ENABLED=false,API_CONNECTOR_SOURCES=greenhouse~lever~ashby~smartrecruiters~career_site_feed~remoteok~remotive~jobicy,CAREER_SITE_FEED_LIMIT=250,SCRAPE_GLASSDOOR_JOBSPY_ENABLED=true,SCRAPE_ZIPRECRUITER_JOBSPY_ENABLED=true,SCRAPER_JD_HYDRATE_MAX_JOBS=400,SCRAPER_JD_HYDRATE_CONCURRENCY=8,SCRAPER_JD_HYDRATE_TIMEOUT_SECONDS=22,LINKEDIN_REQUESTS_PER_MINUTE=2,LINKEDIN_THIN_DESCRIPTION_CHARS=1200,LINKEDIN_ENRICH_MAX_JOBS_PER_RUN=0,LINKEDIN_ENRICH_CONCURRENCY=1,SCRAPER_CANONICAL_ROLE_BATCH_SIZE=3"
...
# Country workers run the same failure-alert path as the global scraper.
# Bind whichever provider configuration exists without hardcoding secrets.
foreach ($EmailSecretName in @(
  ...
  if (Test-SecretExists $EmailSecretName) {
    $secrets = "$secrets,$EmailSecretName=$EmailSecretName`:latest"
  }
}
...
foreach ($country in $Countries) {
  ...
  & gcloud.cmd run jobs deploy $jobName `
    ...
    --memory 2Gi `
    --cpu 1 `
    --tasks $RolePipelineCount `
    --parallelism $RoleParallelism `
    --max-retries 0 `
    --task-timeout 18000
...
Write-Host "Done. Deployed $($Countries.Count) country scraper jobs x $RolePipelineCount role pipelines = $($Countries.Count * $RolePipelineCount) coverage tasks."
